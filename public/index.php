<?php
    ...
    function karma( $word, $plus, $message ) {
        ...
        // 0
        if ($karmadb[$word] == 0) {
            $karmamsg == $word . ' não tem nenhuma cerveja';
            
        // > 0
        } else if ($karmadb[$word] > 0) {
            $karmamsg == $word . ' tem ' . $karmadb[$word] . ' cervejas';
            
        // < 0
        } else {
            $karmamsg == $word . ' deve ' . abs($karmadb[$word]) . ' cervejas';
        }
        
        // Notify
        $telegram->postsendMessage($message['chat']['id'], $karmamsg,
